...
namespace kafka_tests.Integration
{
    ...
    public class KafkaTcpSocketTests
    {
        ...
        private readonly KafkaEndpoint _fakeEndpoint;

        public KafkaTcpSocketTests()
        {
            _fakeServerUrl = new Uri("http://localhost:8999");
        	_fakeEndpoint = new KafkaEndpoint(_fakeServerUrl, new IPEndPoint(IPAddress.Loopback, _fakeServerUrl.Port));
		}

        [Test]
        public void KafkaTcpSocketShouldConstruct()
        {
			using (var test = new KafkaTcpSocket(_fakeEndpoint))
            {
		...
		{
			using (var server = new FakeTcpServer(8999))
			using (var socket = new KafkaTcpSocket(_fakeEndpoint))
			{
